-- [[ JATREX GUI - THE FULL LEGENDARY VERSION ]] --
...
local SplashLogo = Instance.new("ImageLabel", SplashFrame)
SplashLogo.BackgroundTransparency = 1
SplashLogo.Size = UDim2.new(0, 0, 0, 0)
...
-- كل الأزرار موجودة هنا بنفس الترتيب
...
RunService.RenderStepped:Connect(function()
    if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if config.fly or config.fling then
            ...
            if bv.Parent then 
                bv.Parent = nil; hrp.RotVelocity = Vector3.new(0,0,0)
                for _, v in pairs(lp.Character:GetChildren()) do if v:IsA("BasePart") then v.CanCollide = true end end 
            end
        end
...
task.spawn(function()
    ...
    s.SoundId = "rbxassetid://4612374667" -- صوت حماسي (Upee/Action Style)
    s.Volume = 5; s:Play()

    TweenService:Create(SplashLogo, TweenInfo.new(0.8, Enum.EasingStyle.Elastic), {Size = UDim2.new(0, 160, 0, 160), Position = UDim2.new(0.5, -80, 0.45, -80), ImageTransparency = 0}):Play()
    task.wait(0.5)
    TweenService:Create(SplashText, TweenInfo.new(0.8), {TextTransparency = 0}):Play()
    task.wait(2.5)
    SplashFrame:Destroy(); MainFrame.Visible = true; ToggleBtn.Visible = true
end)
