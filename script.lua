-- [[ JATREX GUI - COMPLETE ULTIMATE EDITION (FIXED + DEX) ]] --
...
RunService.Heartbeat:Connect(function()
    if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") and lp.Character:FindFirstChild("Humanoid") then
        ...
        if config.noclip or config.fling then 
        ...
        if config.fling then
            for _, v in pairs(lp.Character:GetChildren()) do
                if v:IsA("BasePart") and v.Name ~= "HumanoidRootPart" then
                    v.RotVelocity = Vector3.new(300000, 300000, 300000)
                    v.Velocity = Vector3.new(500, 500, 500)
                end
            end
